...
//        String body = "{\"content_1\": \"" + face1 + "\", \"content_2\":\"" + face2 + "\", \"type\":\"" + "1" + "\"}";
    ...
    public boolean faceVerify(int type, String face1, String face2, double targetConfidence) {
        ...
        try {
            response = testCompareFace(type, face1, face2);
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        new FaceVerifyUtils().testCompareFace(face3, face4);
...
//    }
    ...
    //阿里云人脸识别测试
    public CompareFaceResponse testCompareFace(int type, String face1 , String face2) throws Exception {
        System.out.println("--------  人脸比对 --------------");
        CompareFaceRequest req = new CompareFaceRequest();

        req.setImageDataA(face1);
        req.setImageDataB(face2);
...
//        printResponse(req.getSysActionName(), resp.getRequestId(), resp);
    ...
    private  <R extends RpcAcsRequest<T>, T extends AcsResponse> T getAcsResponse(R req) throws Exception {
        DefaultProfile profile = DefaultProfile.getProfile(
                "cn-shanghai",             //默认
                "LTAI5tN6eDuPcAmgVPKqKoXy",
                "QzQxSoarlBOQX4EOuSeTo0Y9yOzm9D");
//                aliyunResource.getAccessKeyID(),         //您的AccessKeyID
//                aliyunResource.getAccessKeySecret());    //您的AccessKeySecret
    ...
    public  void printResponse(String actionName, String requestId, AcsResponse  data) {
